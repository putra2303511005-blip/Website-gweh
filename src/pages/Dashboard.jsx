...
import { 
  ...
  Wallet, BrainCircuit, TrendingUp, TrendingDown, Send, Cpu, Wheat
} from "lucide-react";
...
export default function Dashboard() {
  ...
  const handleSubmitSapi = async (e) => {
    e.preventDefault();
    if (!formData.idSapi || !formData.berat || Number(formData.berat) <= 0) {
      return alert("Mohon lengkapi data dengan benar (Berat harus > 0).");
    }

    setIsLoading(true);
    try {
      const submitData = { ...formData, berat: Number(formData.berat) };

      if (isEditing) {
        await updateDoc(doc(db, "sapi", editId), submitData);
      } else {
        const docRef = await addDoc(collection(db, "sapi"), {
          ...submitData, adgTerakhir: 0, 
          tanggalMasuk: new Date().toISOString(), userId: user.uid 
        });
        await addDoc(collection(db, "sapi", docRef.id, "riwayat_timbangan"), {
          berat: Number(submitData.berat), tanggal: new Date().toISOString()
        });
      }
      closeModal();
    } catch (error) { 
      alert("Gagal simpan data sapi! Pastikan koneksi internet stabil."); 
      console.error(error);
    } finally { 
      setIsLoading(false); 
    }
  ...
  const handleAddWeight = async (e) => {
    e.preventDefault();
    const beratBaru = Number(newWeight);
    if (!newWeight || beratBaru <= 0) return alert("Masukkan berat yang valid!");
    ...
    try {
      ...
      if (!querySnapshot.empty) {
        ...
        adg = ((beratBaru - dataTerakhir.berat) / selisihHari).toFixed(2);
      }

      await addDoc(riwayatRef, { berat: beratBaru, tanggal: new Date().toISOString(), adg: Number(adg) });
      await updateDoc(doc(db, "sapi", selectedSapi.idFirebase), { berat: beratBaru, adgTerakhir: Number(adg) });
  ...
  const handleFinance = async (type) => {
    ...
    if (amt === null) return; // User membatalkan prompt
    
    const val = Number(amt);
    if (isNaN(val) || val <= 0) {
      return alert("Nominal tidak valid! Masukkan angka lebih dari 0.");
    }

    try {
      const currentIncome = stats?.totalIncome || 0;
      const currentExpense = stats?.totalExpense || 0;
      
      const newIncome = type === 'Pemasukan' ? currentIncome + val : currentIncome;
      const newExpense = type === 'Pengeluaran' ? currentExpense + val : currentExpense;
  ...
  // --- LOGIKA VET AI ASSISTANT (GEMINI) ---
  const handleAiChat = async (e) => {
    ...
    try {
      ...
      if (!API_KEY) throw new Error("API Key belum disetting di environment (.env)");
      ...
      if (!data.candidates || data.candidates.length === 0) throw new Error("Format respons AI tidak sesuai.");
  ...
  const handleSubmitPakan = async (e) => {
    e.preventDefault();
    const jmlKg = Number(pakanForm.jumlahKg);
    const konKg = Number(pakanForm.konsentrat);

    if (jmlKg <= 0 || konKg < 0) return alert("Masukkan jumlah pakan dengan valid (Minimal 0).");

    setIsLoading(true);
    try {
      await addDoc(collection(db, "pakan"), { 
        ...pakanForm, 
        jumlahKg: jmlKg, 
        konsentrat: konKg, 
      ...
      setPakanForm({ jenisPakan: "Hijauan", jumlahKg: "", konsentrat: "", nutrisi: "Sedang" });
    } catch (error) { 
    ...
    <div className="flex h-screen bg-slate-50 font-sans text-slate-800">
      ...
      <aside className="w-64 bg-white border-r border-slate-200 hidden md:flex md:flex-col shadow-sm z-20">
        ...
        <nav className="p-4 flex-1 space-y-2 overflow-y-auto">
          ...
          <div className="text-xs font-bold text-slate-400 mt-6 mb-2 ml-2 uppercase">Pintar</div>
          ...
          {activeTab === 'dashboard' && (
            <div className="max-w-7xl mx-auto space-y-6">
              <div className="flex justify-between items-center">
                ...
                <button onClick={() => { closeModal(); setIsModalOpen(true); }} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-xl font-bold flex gap-2"><Plus size={18}/> Tambah</button>
              </div>
              ...
              {/* Pastikan komponen Statistik memiliki fallback internal jika dataSapi kosong */}
                        ...
                        <tr key={sapi.idFirebase} className="hover:bg-slate-50">
                          ...
                          <td className="p-4 flex justify-center gap-2">
                            ...
                            <button onClick={() => { 
                                ...
                                const { idFirebase, ...restSapiData } = sapi; // Menghindari penulisan ulang idFirebase ke doc
                                setFormData(restSapiData); 
                                setIsModalOpen(true); 
                              }} className="p-2 text-blue-600 rounded-lg hover:bg-blue-50" title="Edit"><Edit size={16}/></button>
                            <button onClick={async () => { 
                                if(window.confirm("Hapus data sapi ini secara permanen?")) {
                                  try {
                                    await deleteDoc(doc(db, "sapi", sapi.idFirebase));
                                  } catch (err) { alert("Gagal menghapus sapi."); }
                                }
                              }} className="p-2 text-rose-500 rounded-lg hover:bg-rose-50" title="Hapus"><Trash2 size={16}/></button>
                          </td>
                        </tr>
                      ))}
                  </tbody>
              ...
              <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
                ...
                <table className="w-full text-left text-sm">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {pakanLogs.length === 0 ? <tr><td colSpan="5" className="p-6 text-center text-slate-400">Belum ada riwayat pakan.</td></tr> :
                      pakanLogs.map(p => (
             ...
             <div className="max-w-5xl mx-auto space-y-6">
                ...
                <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden mt-4">
                  ...
                  <div className="divide-y divide-slate-100">
                    {transactions.length === 0 ? <div className="p-6 text-center text-slate-400">Belum ada transaksi tercatat.</div> :
                      transactions.map(t => (
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white p-6 rounded-2xl w-full max-w-md shadow-xl">
            ...
            <form onSubmit={handleSubmitSapi}>
